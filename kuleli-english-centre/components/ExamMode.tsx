...
import { Mic, Play, ArrowRight, CheckCircle, AlertCircle, Volume2, Clock, GraduationCap, FastForward, TrendingUp, AlertTriangle, RefreshCw, Trophy, ClipboardCheck, ArrowUpRight, MessageSquare, XCircle, Info, Volume1, Flag } from 'lucide-react';
...
const ExamMode: React.FC<ExamModeProps> = ({ initialPart = ExamPart.INTRO, isStandalone = false, onModeChange }) => {
  ...
  const handleSubmission = async (audioBlob: Blob) => {
    try {
      ...
      const result = await processStudentInput(audioBlob, 'EXAM', context, [], part);
      
      turnCountRef.current += 1;
      setCurrentFeedback(result.feedback); 

      if (result.moveNext || autoAdvanceRef.current) {
             ...
             <div className="space-y-6">
                ...
                <div className="space-y-4">
                   {allMistakes.length > 0 ? (
                     allMistakes.map((m, i) => (
                       <div key={i} className={`bg-white border rounded-2xl overflow-hidden shadow-sm flex flex-col md:flex-row ${m.type === 'pronunciation' ? 'border-blue-100' : 'border-slate-100'}`}>
                          <div className={`flex-1 p-4 border-r border-slate-100 ${m.type === 'pronunciation' ? 'bg-blue-50/50' : 'bg-red-50/50'}`}>
                             <div className="flex items-center justify-between mb-1">
                                <div className={`text-[10px] font-black uppercase ${m.type === 'pronunciation' ? 'text-blue-400' : 'text-red-400'}`}>
                             ...
                             <div className={`text-sm italic ${m.type === 'pronunciation' ? 'text-blue-900' : 'text-red-900'}`}>"{m.mistake}"</div>
                          </div>
                          <div className={`flex-1 p-4 ${m.type === 'pronunciation' ? 'bg-indigo-50/50' : 'bg-green-50/50'}`}>
                             <div className={`text-[10px] font-black uppercase mb-1 ${m.type === 'pronunciation' ? 'text-indigo-400' : 'text-green-400'}`}>Correct:</div>
                             <div className={`text-sm font-bold ${m.type === 'pronunciation' ? 'text-indigo-900' : 'text-green-900'}`}>"{m.correction}"</div>
                          </div>
                       </div>
                     ))
                   ) : (
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {currentFeedback.mistakesAndCorrections.map((m, i) => (
                       <div key={i} className={`p-3 rounded-xl border flex flex-col gap-1 ${m.type === 'pronunciation' ? 'bg-blue-50 border-blue-100' : 'bg-red-50 border-red-100'}`}>
                          <div className="flex items-center justify-between">
                             <span className={`font-black uppercase text-[9px] ${m.type === 'pronunciation' ? 'text-blue-400' : 'text-red-400'}`}>{m.type === 'pronunciation' ? 'Pronunciation' : 'Grammar'}:</span>
                             {m.type === 'pronunciation' && (
                                <button onClick={() => playPronunciationAudio(m.correction)} className="text-blue-600"><Volume1 size={14} /></button>
                             )}
                          </div>
                          <span className={`${m.type === 'pronunciation' ? 'text-blue-900' : 'text-red-900'} italic text-[11px]`}>"{m.mistake}"</span>
                          <span className={`font-black uppercase text-[9px] mt-1 ${m.type === 'pronunciation' ? 'text-indigo-400' : 'text-green-400'}`}>Correction:</span>
                          <span className={`${m.type === 'pronunciation' ? 'text-indigo-800' : 'text-green-800'} font-bold text-[11px]`}>"{m.correction}"</span>
                       </div>
                    ))}
                  </div>
                </div>
        ...
        <div className="flex-[2] md:max-w-sm flex flex-col gap-4">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col gap-4">
            ...
            <div className="space-y-3 pb-2">
              ...
              {/* Finish Willingly Button: Available in All modes whenever a turn has been made */}
              {status === ExamStatus.USER_SPEAKING && !isRecording && turnCountRef.current > 0 && (
                <button 
                  onClick={handleFinishWillingly} 
                  className="w-full py-4 bg-slate-800 text-white rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-slate-900 transition-colors shadow-lg border-2 border-slate-700"
                >
                  <Flag size={18} />
