...
const ExamMode: React.FC<ExamModeProps> = ({ initialPart = ExamPart.INTRO, isStandalone = false, onModeChange, user }) => {
  ...
  useEffect(() => {
    ...
    if (initialPart === ExamPart.INTRO) {
      ...
      msg = "Welcome. We will now begin Part 2: Picture Description. Please wait while I prepare the image for you.";
    } else {
  ...
  const setupNextPart = async (targetPart: ExamPart) => {
    ...
    try {
      if (targetPart === ExamPart.PICTURE) {
        // Updated Part 2 Logic: Single Image
        const topic = await generateExamTopic(2);
        secretTopicRef.current = topic;
        
        const instructions = `Part 2: Picture Description. I will show you a picture. Please describe it in detail. Talk about the people, the place, and the actions. You have 1 minute to prepare.`;
        setAiMessage(instructions);
        
        // Generate single image
        const img = await generateExamImage(topic);
        setExamImages(img ? [img] : []);
  ...
  const handleSubmission = async (audioBlob: Blob) => {
    try {
      ...
      // Updated Context for Part 2 Description
      if (part === ExamPart.PICTURE) context = `Part 2: Picture Description. Image Topic: ${secretTopicRef.current}. Student should describe the people, actions, and setting in the picture.`;
  ...
  const calculateTotalScore = () => {
    return (Object.values(resultsHistory) as FeedbackData[]).reduce((acc, f) => {
      // Add vocabularyScore to the total
      return acc + f.taskAchievementScore + f.pronunciationScore + f.grammarScore + f.fluencyCoherenceScore + (f.vocabularyScore || 0);
    }, 0);
  };
...
${(Object.values(resultsHistory) as FeedbackData[]).map((f, i) => `Part ${i+1}: ${f.feedbackText}`).join('\n\n')}
      ...
      <div className="flex items-center justify-center min-h-full bg-slate-50/50 p-6">
        <div className="bg-white p-8 rounded-3xl shadow-xl border border-slate-100 max-w-md w-full text-center space-y-6">
          ...
          <div className="text-slate-500 text-sm text-left bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-2">
             <p className="font-bold text-slate-700">Grading System (New Rubric):</p>
             <ul className="space-y-1 list-disc pl-4 text-xs">
               <li>Part 1: 30 points max (6 pts per category)</li>
               <li>Part 2: 40 points max (8 pts per category)</li>
               <li>Part 3: 30 points max (6 pts per category)</li>
               <li>Categories: Task, Fluency, Pronunciation, Vocabulary, Grammar</li>
          ...
          <div className="p-8 md:p-12 space-y-12">
             <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[1, 2, 3].map(p => {
                  ...
                  if (!res) return null;
                  const pScore = res.taskAchievementScore + res.pronunciationScore + res.grammarScore + res.fluencyCoherenceScore + (res.vocabularyScore || 0);
                  const pMax = p === 2 ? 40 : 30; // Updated Max scores
                  return (
             ...
             <div className="space-y-6">
                ...
                {/* Updated to grid-cols-2 to fit 5 items, last one will naturally wrap or we can use 3 cols on large screens */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                   <StatBar label="Task Achievement" current={(Object.values(resultsHistory) as FeedbackData[]).reduce((a, b) => a + b.taskAchievementScore, 0)} total={20} />
                   <StatBar label="Fluency & Coherence" current={(Object.values(resultsHistory) as FeedbackData[]).reduce((a, b) => a + b.fluencyCoherenceScore, 0)} total={20} />
                   <StatBar label="Pronunciation" current={(Object.values(resultsHistory) as FeedbackData[]).reduce((a, b) => a + b.pronunciationScore, 0)} total={20} />
                   <StatBar label="Vocabulary" current={(Object.values(resultsHistory) as FeedbackData[]).reduce((a, b) => a + (b.vocabularyScore || 0), 0)} total={20} />
                   <StatBar label="Grammar" current={(Object.values(resultsHistory) as FeedbackData[]).reduce((a, b) => a + b.grammarScore, 0)} total={20} />
                </div>
             </div>
  ...
  if (status === ExamStatus.PART_COMPLETED && currentFeedback) {
    const pScore = currentFeedback.taskAchievementScore + currentFeedback.pronunciationScore + currentFeedback.grammarScore + currentFeedback.fluencyCoherenceScore + (currentFeedback.vocabularyScore || 0);
    const pMax = part === ExamPart.PICTURE ? 40 : 30; // Updated max scores per part
    const maxCatScore = part === ExamPart.PICTURE ? 8 : 6;
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-8 animate-in slide-in-from-bottom-8 duration-500">
        <div className="bg-white rounded-[40px] shadow-xl border border-slate-100 overflow-hidden">
           ...
           <div className="p-8 md:p-12 space-y-10">
              ...
              {/* Updated to grid-cols-2 or 3 to fit 5 items */}
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                 <MiniScoreItem label="Task" score={currentFeedback.taskAchievementScore} max={maxCatScore} />
                 <MiniScoreItem label="Fluency" score={currentFeedback.fluencyCoherenceScore} max={maxCatScore} />
                 <MiniScoreItem label="Vocab" score={currentFeedback.vocabularyScore || 0} max={maxCatScore} />
                 <MiniScoreItem label="Grammar" score={currentFeedback.grammarScore} max={maxCatScore} />
                 <MiniScoreItem label="Pronunc." score={currentFeedback.pronunciationScore} max={maxCatScore} />
              </div>
        ...
        <div className="flex-[3] bg-white rounded-3xl shadow-sm border border-slate-100 p-6 flex flex-col items-center justify-start min-h-[400px]">
          ...
          {part === ExamPart.PICTURE && examImages.length > 0 && (
            <div className="w-full flex justify-center mb-6">
               <div className="aspect-[4/3] w-full max-w-md overflow-hidden rounded-2xl shadow-lg ring-1 ring-slate-200 bg-slate-100 relative">
                  <img src={examImages[0]} alt="Exam Subject" className="w-full h-full object-cover" />
               </div>
            </div>
          )}
