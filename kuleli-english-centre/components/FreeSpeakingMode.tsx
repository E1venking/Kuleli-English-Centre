...
const FreeSpeakingMode: React.FC = () => {
  ...
  const handleAudioSubmit = async (blob: Blob) => {
    ...
    try {
        ...
        const result = await processStudentInput(blob, 'FREE', undefined, history);
        addMessage('user', "(Audio Input)", undefined); 
        addMessage('ai', result.reply, result.feedback);
    } catch (error: any) {
                  ...
                  return (
                      <div className="space-y-6">
                          <div className="grid grid-cols-2 gap-3">
                              <ScoreCard label="Fluency" score={latestFeedback.fluencyScore} />
                              <ScoreCard label="Grammar" score={latestFeedback.grammarScore} />
                              <ScoreCard label="Vocab" score={latestFeedback.vocabularyScore} />
                              <ScoreCard label="Idioms" score={latestFeedback.idiomScore} />
                          </div>
...
const ScoreCard = ({ label, score }: { label: string, score: number }) => (
    <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 text-center md:text-left">
        ...
        <div className="text-lg md:text-xl font-black text-red-700">{score}<span className="text-[10px] text-slate-400 font-normal">/100</span></div>
    </div>
);
