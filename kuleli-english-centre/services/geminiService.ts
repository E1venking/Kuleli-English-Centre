...
export const generateExamTopic = async (part: number): Promise<string> => {
  return fetchWithRetry(async () => {
    ...
    try {
        ...
        return response.text?.replace(/\*/g, '').trim() || "A student studying in a library";
    } catch (e) {
        if (isQuotaError(e)) {
  ...
  }).catch(() => "A student studying in a library");
};
  ...
  return fetchWithRetry(async () => {
      ...
      const analysisPromise = (async () => {
          ...
          const config = { 
            ...
            responseSchema: {
                ...
                required: ["taskAchievementScore", "pronunciationScore", "grammarScore", "fluencyCoherenceScore", "vocabularyScore", "feedbackText", "mistakesAndCorrections"]
            }
          };
    ...
    const config = { 
        ...
        responseSchema: {
          ...
          required: ["replyText", "taskAchievementScore", "pronunciationScore", "grammarScore", "fluencyCoherenceScore", "vocabularyScore", "feedbackText", "examMoveToNext", "mistakesAndCorrections"]
        }
    };
